...
public class TraineeController {
    ...
    @GetMapping("/traineesByTrainerId")
    public ResponseEntity<List<TraineeDto>> findTraineesByTrainerId (@RequestParam("trainerId") Long trainerId){
        if (trainerId != null){
            return ResponseEntity.ok(traineeService.findTraineeByTrainerId(trainerId));
        }
        return new ResponseEntity<>(HttpStatusCode.valueOf(HttpStatus.NOT_FOUND.value()));
    }

    @GetMapping("/trainerByTraineeId")
    public ResponseEntity<?> findTrainerByTraineeId (@RequestParam("traineeId") Long traineeId){
            Trainer trainer = traineeService.findTrainerByTraineeId(traineeId);
    ...
    @PostMapping("{traineeId}/add-exercise/{exerciseId}")
    public ResponseEntity<?> addExerciseToTrainer(@PathVariable Long traineeId, @PathVariable Long exerciseId){
        TraineeDto traineeDto = traineeService.addExerciseToTrainee(traineeId, exerciseId);
        if (traineeId != null){
            return ResponseEntity.ok(traineeDto);
        }
        return new ResponseEntity<>(HttpStatusCode.valueOf(HttpStatus.NOT_FOUND.value()));
